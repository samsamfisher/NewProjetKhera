      ...
      detectChangeEnd() && document.querySelector(sliderElement).addEventListener(detectChangeEnd(), function () {
        if (isChanging) {
          setTimeout(function() {
            ...
            if (document.querySelector('.slider__page--active')) {
              document.querySelector('.slider__page--active').classList.remove('slider__page--active');
            }
            document.querySelector('[data-slider-index="' + currentSlide + '"]').classList.add('slider__page--active');
          }, 400);
        }
      });
